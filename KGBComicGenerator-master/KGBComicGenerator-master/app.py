...
def gpt_prompt_and_eval(input_places, input_probs, tags, entropy_specifier, initial_state):
    ...
    prompt_init = "Aravind is trapped in a flight in mid-air, hijacked by five terrorists. However, he is a brave and shrewd guy who is capable of overpowering them and saving the innocent passengers and pilots. Explain how he saves them in 3 steps and be descriptive. "
    prompt_init += "He considers planning carefully step by step, one at a time. Use grandiose language. Embelish everything and paint a picture with words. Make the descriptions drip with imagery. Make the image ultra high resoulution. Give the detailed image dont leave any details "
    ...
    response = openai.Completion.create(
        model="text-davinci-003", prompt=prompt_init, temperature=0, max_tokens=1000)

    f = open("./results/response.txt", "a")
...
def image_from_response(storyline, pre_prompt):
    # split the text
    ...
    for step in range(len(processed)):
        ...
        with open('./results/pic'+str(step)+'.png', 'wb') as handler:
            handler.write(img_data)
        time.sleep(5)

    # return list of image urls and corresponding text as a dictionary
    return {"urls": img_urls, "text": processed}
...
def full_circ_instance(verbose, sim_type, N_qubits, all_places, start, steps, activate_ai, pre_prompt):
    # provider
    ...
    if sim_type == 'ideal':
        ...
        # get least_busy backend with required number of qubits
    ...
    PolarPlotmaker(processed_vals, labels=list_places, figsize=(5, 5), dpi=120, background=None, debug=False, tick_color='chartreuse',
                   linelength=10, pad=8, save_name='./results/roseplot', show=False, has_error=True, offwhite_cutoff=170, labelsize=8)
    # this takes into
    # list_places
    # processed_vals
    # saves picture into results/roseplot.png
    ...
    if activate_ai:
        ...
        # image_from_response(storyline) returns {"urls": img_urls, "text": processed}
        return image_from_response(storyline, pre_prompt=pre_prompt)
...
def home():
    ...
    mystrinf = '[' + request.form['my_list'] + ']'
    my_list = ast.literal_eval(mystrinf)
    pre_prompt = request.form['pre_prompt']
    data = full_circ_instance(verbose=True, sim_type='ideal',
                       N_qubits=num_qubits,all_places=my_list, start=3, steps=2, activate_ai=True, pre_prompt=pre_prompt)
    return render_template('comic.html', urls=data['urls'], text=data['text'])    
